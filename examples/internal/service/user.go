...
func (s *UserService) Create(ctx context.Context, req userv1.CreateReq) (userv1.CreateRes, error) {
	...
	if len(req.RoleIDs) > 0 {
		roles, err := s.rbac.FindRolesByIDs(ctx, req.RoleIDs)
		if err != nil {
			return userv1.CreateRes{}, err
		}
		if err := s.users.ReplaceRoles(ctx, user, roles); err != nil {
			return userv1.CreateRes{}, err
		}
	}
	...
	roleItems := make([]rbacv1.RoleRes, 0, len(created.Roles))
...
func (s *UserService) Update(ctx context.Context, id uint, req userv1.UpdateReq) (userv1.UpdateRes, error) {
	...
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return userv1.UpdateRes{}, gorm.ErrRecordNotFound
	}
	if err != nil {
	...
	roleItems := make([]rbacv1.RoleRes, 0, len(updated.Roles))
